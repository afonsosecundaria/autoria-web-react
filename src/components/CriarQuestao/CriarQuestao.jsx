...
export default function CriarQuestao() {
  const { idTopico } = useParams(); // vem da URL
  ...
  function salvarQuestao(e) {
    ...
    fetch("https://autoria-web-react-production.up.railway.app/api/questoes", {
      ...
      .then(data => {
        alert(data.message);
        setEnunciado(""); setA(""); setB(""); setC(""); setD("");
      });
  }
  ...
  return (
    <Layout>
      ...
      <form onSubmit={salvarQuestao}>
        ...
        <textarea placeholder="Enunciado" value={enunciado}
          onChange={e => setEnunciado(e.target.value)} required />
